    ...
    const moviePlaceHolder = ()=>{
        ...
        .then(content =>{
            ...
            ticketBuy.addEventListener('click', ()=>{
                ...
                if(tickets <= 0){
                    const firstMovie = document.getElementById("1")
                    firstMovie.innerHTML = `${firstMovie.title} <span class = "badge bg-danger me-1">SSOLD OUT</span`
    ...
    const movieDetails = ()=>{
        ...
        .then(data=>{
            ...
            for(let i = 0; i < filmData.length; i++){
                ...
                movieList.classList.add("list-group-item", "border", "border-info", "cinema")

                movieList.setAttribute('id', `${item.id}`)
                // movielist.className = "cinema"
